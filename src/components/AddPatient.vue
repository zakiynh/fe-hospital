...
export default defineComponent({
    ...
    setup() {
        ...
        function addPatient() {
            axios.post('http://localhost:8000/api/v1/post', formInput)
                .then(({data}) => {
...
<template>
    <div>
        <Dialog v-model:visible="visible" modal header="Add New Patient" :style="{ width: '45vw' }" :breakpoints="{ '960px': '75vw', '641px': '100vw' }">
            <form @submit.prevent="addPatient">
                ...
                <div class="flex justify-content-center gap-2">
                    <Button type="button" label="Cancel" size="small" icon="pi pi-times" class="mt-3" @click="$emit('closeAdd')" severity="danger"/>
                    <Button type="submit" label="Add Patient" size="small" icon="pi pi-user-plus" class="mt-3" severity="info"/>
                </div>
            </form>
        </Dialog>
    </div>
